...
def pSTAT_response(test_doses, tag):
    ...
    if not os.path.exists(dir):
        os.makedirs(dir)
    AP_AV_simulations(USP18_sf, TIMES, test_doses, dir, tag=tag)


def AP_AV_fitting(simulate_pSTAT, fit, plot):
    # -------------------------------------------------------------------------
    # -------------------------------------------------------------------------
    ...
    if plot:
        ...
        # save figure
        plt.tight_layout()
        fig.savefig(os.path.join(dir, 'Figure_6_fit.pdf'))
...
if __name__ == '__main__':
    ...
    fit = True
    plot = True
    AP_AV_fitting(simulate_pSTAT, fit, plot)
