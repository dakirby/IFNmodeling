...
# Initialize PySB model object for running simulations.  Simulation timespan
# should match experimental data.
...
# The "experimental data" is the TotalpSTAT trajectory for Alpha, then Beta, at
# each dose.
...
# Create lists of sampled pysb parameter names to use for subbing in parameter
# values in likelihood function.
pysb_sampled_parameter_names = ['kpa', 'kSOCSon', 'R1', 'R2', 'ka1', 'ka2',
                                'k_a1', 'k_a2', 'kint_a', 'kint_b', 'krec_a2',
                                'krec_b2']

# Define likelihood function to generate simulated data that corresponds to
# experimental time points.
# This function should take as input a parameter vector (parameter values are
...
def likelihood(parameter_vector,
    ...
    shared_param_dict = {pname: 10 ** pvalue for pname, pvalue in
                         zip(sampled_parameter_names, parameter_vector)}

    model.set_parameters(shared_param_dict)

    # Simulate experimentally measured TotalpSTAT values.
    # Alpha
    dradf = model.doseresponse(tspan, 'TotalpSTAT', 'Ia', alpha_doses,
                               parameters={'Ib': 0}, scale_factor=sf,
                               return_type='dataframe',
                               dataframe_labels='Alpha')
    drbdf = model.doseresponse(tspan, 'TotalpSTAT', 'Ib', beta_doses,
                               parameters={'Ia': 0}, scale_factor=sf,
                               return_type='dataframe',
...
def DREAM_fit(niterations, priors_list):
    converged = False
    ...
    sampled_params, log_ps = run_dream(priors_list, likelihood,
    ...
    try:
        ...
        for dim in range(ndims):
            ...
            fig.savefig(os.path.join(save_dir, sim_name + '_dimension_' +
                                     str(dim) + '_' +
                                     pysb_sampled_parameter_names[dim] +
                                     '.pdf'))

        # Convert back to true value rather than log value
        # converted_samples = np.power(np.multiply(np.ones(np.shape(samples)),
        # 10), samples)
...
if __name__ == '__main__':
    # Add vector of PySB rate parameters to be sampled as unobserved random
    # variables to DREAM with log normal priors.
    ...
    # Run PyDREAM
    DREAM_fit(niterations=niterations, priors_list=priors_list)
