...
IFN_exps = [data.loc[(data.loc[:,'Dose (pM)']==10) & (data.loc[:,'Interferon']=="Alpha"),['0','5','15','30','60']].values[0],
            ...
            data.loc[(data.loc[:,'Dose (pM)']==11000) & (data.loc[:,'Interferon']=="Beta"),['5','15','30']].values[0]]
...
IFN_sigmas =[data.loc[(data.loc[:,'Dose (pM)']==10) & (data.loc[:,'Interferon']=="Alpha_std"),['0','5','15','30','60']].values[0],
             ...
             data.loc[(data.loc[:,'Dose (pM)']==11000) & (data.loc[:,'Interferon']=="Beta_std"),['5','15','30']].values[0]]
...
# =============================================================================
def logp_helper(gamma, all_sims):
    logp = 0
...
def get_likelihood_logp(fit_list):
    ...
    alpha_parameters[I_index_Alpha] = NA*volEC*4000E-12
    (_, sim) = alpha_mod.simulate([15*60,30*60,60*60], param_values=alpha_parameters)
    all_sims.append(sim['TotalpSTAT'])
    beta_parameters[I_index_Beta] = NA*volEC*2000E-12
    (_, sim) = beta_mod.simulate([5*60,15*60,30*60,60*60], param_values=beta_parameters)
    all_sims.append(sim['TotalpSTAT'])

    alpha_parameters[I_index_Alpha] = NA*volEC*8000E-12
    (_, sim) = alpha_mod.simulate([15*60,30*60,60*60], param_values=alpha_parameters)
    all_sims.append(sim['TotalpSTAT'])
    beta_parameters[I_index_Beta] = NA*volEC*11000E-12
    (_, sim) = beta_mod.simulate([5*60,15*60,30*60], param_values=beta_parameters)
    all_sims.append(sim['TotalpSTAT'])
    ...
    opt = minimize(logp_helper,[40],args=(all_sims))
    gamma = opt['x'].item()
    logp = opt['fun']
    return [logp, gamma]
...
def get_acceptance_rate(theta, priors_dict, beta, rho):
    ...
    for i in range(100):
        ...
        if new_score!=-1 and (new_score < old_score or np.random.rand() < np.exp(-(new_score-old_score))*asymmetry_factor):
...
def mh(ID, jobs, result, countQ):
    while True:
        ...
        progress_bar = n/10
        while acceptance<n:
            print("Chain {}: {}".format(ID,acceptance))
...
#        'krec_b1':[1E-7,1E-1,None,None],'krec_b2':[1E-6,1E0,None,None]}
    ...
    #    pflag=True, cpu=None, randomize=True)
    MCMC(500, p0, our_priors_dict, 50, 1, 6, burn_rate=0.2, down_sample=8, max_attempts=6)
